...
parser.add_argument('--save_interval', type=int, default=9999999, help='save interval')
...
parser.add_argument('--dataset_name', type=str, default='e2e')
...
def optimizer_step(_loss, _optimizer, _model, _schedule, args, is_update=True, is_lr_update=True,tr_iter=None): # _input=None, _target=None, _msk=None,
    if args.fp16:
    ...
    if is_update:
        ...
        if is_lr_update:
            lr_parabola(_model, _optimizer, _loss, args.grad_acc, tr_iter= tr_iter, device=args.device, task='NLG')                
...
def train_validate(
    ...
    for idx, data in enumerate(train_loader):
        ...
        is_lr_update = True if train_step % (args.grad_acc*args.lazy_freq) == 0 else False 
        
        avg_lm_loss.update(_lm_loss.item())
        optimizer_step(
            _lm_loss/(args.grad_acc), optimizer, model, scheduler, args, is_update=is_update, is_lr_update=is_lr_update, tr_iter=tr_iter
        )
        ...
        if train_step % args.save_interval == 0: 
            if args.rank == 0:
                model_path = os.path.join(args.work_dir, f'model_{args.model_card}_{args.scheduler}.{train_step}.pt')
